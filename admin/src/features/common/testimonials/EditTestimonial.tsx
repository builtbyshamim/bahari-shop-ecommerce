...
import { useUpdateTestimonialMutation } from './testimonialsApi';
...
const EditTestimonial = ({ testimonial, onClose }: { testimonial: any; onClose: () => void }) => {
  const {
    register,
    handleSubmit,
    setValue,
    ...
    defaultValues: {
      ...
      productLabel: testimonial?.productLabel || '',
      colorIndex: testimonial?.colorIndex ?? 0,
  ...
  useEffect(() => {
    if (testimonial) {
      ...
      setPreview(testimonial.avatarUrl || null);
    }
  }, [testimonial, reset]);
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      const result = await updateTestimonial({ id: testimonial.id, formData }).unwrap();
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="flex items-center gap-4">
          <div className={`w-16 h-16 rounded-full bg-gradient-to-br ${colorClass} flex items-center justify-center text-white text-lg font-bold shrink-0 overflow-hidden`}>
            {preview ? (
              ...
              <span>{autoInitials(nameValue)}</span>
            )}
          </div>
      ...
      <InputString name="avatarInitials" label="Avatar Initials" placeholder="e.g. RI" register={register} errors={errors} required={false} />
      ...
      {/* Rating */}
      <div>
        <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">Rating</label>
        <select {...register('rating')} className="w-full border border-gray-200 bg-gray-50 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
          {[5, 4, 3, 2, 1].map((r) => (
      ...
      <div>
        ...
        <textarea {...register('review', { required: 'Review is required' })} rows={3}
      ...
      {/* Color picker */}
      <div>
        <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">Avatar Color Theme</label>
        <div className="flex flex-wrap gap-2">
          {COLORS.map((c) => (
            <button key={c.value} type="button" onClick={() => setValue('colorIndex', c.value)}
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1">
          <ToggleSwitch name="isActive" label="Active" register={register} errors={errors} defaultValue={testimonial?.isActive ?? true} onToggle={(val) => setValue('isActive', val)} />
        </div>
      </div>
      ...
      <button type="submit" disabled={isLoading}
        ...
        {isLoading ? (
          ...
          <><FiSave className="mr-2" />Update Testimonial</>
        )}
      </button>
    </form>
  );
