...
import { MdArrowBack, MdTrendingUp, MdCategory, MdReceipt } from 'react-icons/md';
...
export default function IncomeReportPage() {
  ...
  const { data, isFetching } = useGetTransactionsReportQuery({ ...range, type: 'income' });
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            All income transactions with category breakdown
          </p>
        </div>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
          ...
          <ReportKpiCard
            label="Categories"
            value={summary.byCategoryIncome?.length ?? 0}
            icon={<MdCategory />}
            color="purple"
          />
        </div>
      ...
      {/* Category Breakdown */}
      {summary?.byCategoryIncome?.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 p-5 mb-5">
          <h3 className="text-sm font-semibold text-gray-700 mb-3">Income by Category</h3>
          <div className="space-y-2">
            {summary.byCategoryIncome.map((c: any) => {
              const pct =
                summary.totalIncome > 0 ? ((c.total / summary.totalIncome) * 100).toFixed(1) : 0;
              return (
                <div key={c.category}>
      ...
      <div className="table-container">
        <div className="mb-4">
          <ReportExportBar
            onExcelExport={() => canExport && exportTransactionsExcel(data?.data, label, 'income')}
            onPdfExport={() => canExport && exportTransactionsPdf(data?.data, label, 'income')}
            disabled={!canExport}
